...
ggplot(sexe_summary, aes(x = Sexe, y = effectif, fill = Sexe)) +
  ...
  coord_cartesian(ylim = c(0, 2000)) +   # 👈 MODIFIE ICI selon ton besoin
  labs(
...
obtenir_top_diagnostics <- function(data, n = 10) {
  resultat <- data %>%
    count(DGN_PAL, name = "Occurrences") %>%  
...
obtenir_diagnostics_avec_noms <- function(data, n = 10) {
  ...
  top_diagnostics <- top_diagnostics %>%
    rowwise() %>%
    mutate(Signification = recuperer_definition(DGN_PAL)) %>%
    ungroup()
...
print(resultat)
>>>>>>> cdff9d62e1cfb732513d822a86dce1d90d174cd9
```
...
Le test de Wilcoxon compare la distribution du nombre de séjours entre les hommes et les femmes. Avec une p value de 0,24, le test n'est pas significatif au seuil de 5 %. Cela signifie qu'il n'existe pas de preuve statistique d'une différence de comportement de suivi entre les deux sexes.

Le sexe du patient ne semble pas être un facteur influençant l'intensité du parcours de soins post-greffe (nombre de réhospitalisations ou de consultations). Les hommes et les femmes reçoivent, en moyenne, un nombre de soins équivalent dans cette cohorte.
...
Observation : Les hommes de l'échantillon sont en moyenne légèrement plus âgés que les femmes (environ 1,7 an de différence). Les médianes confirment cette tendance (57 ans pour les hommes contre 55 ans pour les femmes).
...
Interprétations : Le code **Z940** est l'étiquette dominante pour toutes les tranches d'âge. Les codes **Z512** et **RSSABS** complètent régulièrement le podium, montrant une certaine uniformité dans le codage des dossiers, peu importe l'âge.

On remarque aussi que pour les tranches d'âges élevées( plus de 70 ans), de nouveaux codes apparaissent dans le Top 5 (comme Z491 ou T861), ce qui n'est pas le cas chez les plus jeunes.
...
Il n'y a pas de lien statistiquement significatif entre l'âge du patient et son diagnostic principal vu que le test n'est pas significatif (La valeur de la p-value). Les motifs de prise en charge sont distribués de manière similaire à travers toutes les tranches d'âge.
