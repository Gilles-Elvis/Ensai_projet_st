...
```{r}
p1 <- dist_sejours <- table(table(data$study_id))

barplot(
  dist_sejours,
  xlab = "Durée totale de séjours par patient",
  ylab = "Nombre de patients",
  main = "Distribution  de séjours par patient",
  col = "steelblue"
)
...
ggsave(
  "figures/delai_sejours_suivi_anexe.png",
...
ggplot(greffes_par_region, aes(x = reorder(region, nb_greffes), y = nb_greffes)) +
  geom_bar(stat = "identity", fill = "steelblue", color = "black", alpha = 0.8) +
  # Étiquette principale : nb greffes + pourcentage
  geom_text(
    aes(label = paste0(nb_greffes, " (", pourcentage, "%)")),
    hjust = -0.1, size = 2.5, fontface = "bold"
  ) +
  # Étiquette secondaire : nb établissements (plus petite, en dessous)
  geom_text(
    aes(label = paste0(nb_etablissements, " étab.")),
    hjust = -0.1, vjust = 2.0, size = 2.0, fontface = "bold"
  ) +
  coord_flip() +
  labs(
