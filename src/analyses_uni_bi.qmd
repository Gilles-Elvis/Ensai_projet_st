...
ggplot(sexe_summary, aes(x = Sexe, y = effectif, fill = Sexe)) +
  ...
  #coord_cartesian(ylim = c(0, 4000)) +   # 👈 MODIFIE ICI selon ton besoin
  labs(
...
ggplot(top10_etab, aes(x = reorder(etab_greffe, nb_greffes), y = nb_greffes)) +
  ...
  coord_flip() +
  labs(
    title = "Établissements réalisant le plus de greffes rénales",
    x = "Établissement",
    y = "Nombre de greffes"
  ) +
  theme_minimal()
...
greffes_par_region <- data_patients %>%
  ...
  summarise(
    nb_greffes = n(),
    nb_departements = n_distinct(DEP_greffe),
    nb_etablissements = n_distinct(ETA_NUM_greffe)
  ) %>%
  ungroup() %>%
  arrange(desc(nb_greffes)) %>%
