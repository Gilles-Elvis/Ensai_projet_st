...
```{r}
# Fonction robuste pour détecter les valeurs manquantes
taux_na <- function(x){

  # Si la colonne est caractère, on nettoie l'encodage
  if (is.character(x)) {
    x <- iconv(x, from = "", to = "UTF-8", sub = "")
  }

  mean(is.na(x) | (is.character(x) & x == "")) * 100
}

# Construction du tableau des variables
table_variables <- data.frame(
  variable = names(data),
  type_variable = sapply(data, function(x) class(x)[1]),
  taux_manquant = sapply(data, taux_na)
)

# Arrondir les pourcentages
table_variables$taux_manquant <- round(table_variables$taux_manquant, 2)

# Colonnes pour compléter le tableau
table_variables$unite <- NA
table_variables$libelle <- table_variables$variable

# Tri par taux de données manquantes
table_variables <- table_variables[order(-table_variables$taux_manquant), ]
...
15303 patients pour 22132 lignes donc il y a des patients qui ont plusieurs séjours
...
```{r}
p <- ggplot(age_summary, aes(x = tranche_age, y = n)) +
  ...
  theme_minimal() +
  theme(
    panel.grid = element_blank(),      # supprime tout le quadrillage
    axis.line = element_blank()        # supprime les lignes des axes
  )

ggsave(
  "figures/tranche_age.png",
  plot = p,
  width = 8,
  height = 6,
  dpi = 300
...
Procédons maintenant à des visualisations pour mieux appréhender l'information :

Distrubution du nombre de séjours par patient

```{r}
ggplot(bilan_par_patient, aes(x = nb_sejours)) +
  geom_histogram(binwidth = 1, fill = "steelblue", color = "black") +
  coord_cartesian(xlim = c(0, 50)) +
  labs(
    title = "Distribution du nombre de séjours par patient",
    x = "Nombre de séjours",
    y = "Nombre de patients"
  ) +
  theme_classic()
ggsave("nb_sejours.png", width = 10, height = 6, dpi = 300)
```

Distribution de la durée totale cumulée de séjour par patient

```{r}
ggplot(bilan_par_patient, aes(x = duree_totale_cumulee)) +
  geom_histogram(binwidth = 10, fill = "steelblue", color = "black") +
  coord_cartesian(xlim = c(0, 500)) +
  labs(
    title = "Distribution de la durée totale cumulée de séjour par patient",
    x = "Durée totale cumulée (jours)",
    y = "Nombre de patients"
  ) +
  theme_classic()
ggsave("som_sejours.png", width = 10, height = 6, dpi = 300)
```

Quelques statistiques sur la durée cumulée et le nombre de séjour

```{r}
summary(bilan_par_patient$nb_sejours)
```

```{r}
summary(bilan_par_patient$duree_totale_cumulee)
```

```{r}
round(sd(bilan_par_patient$nb_sejours, na.rm = TRUE), 2)
```

```{r}
round(sd(bilan_par_patient$duree_totale_cumulee, na.rm = TRUE), 2)
```

### Densité des séjours

```{r}
str(data$date_greffe)
str(data$debut_sejour)
```
...
delais_tranches <- table(
  cut(data$delai_greffe_suivi,
...
df_plot$classe_delai <- factor(
  ...
  levels = c("0-7j", "8-30j", "1-3mois",
             "3-6mois", "6-12mois", ">12mois")
)

p <- ggplot(df_plot, aes(x = classe_delai, y = nb_sejours)) +
  geom_col(fill = "steelblue", color = "black", alpha = 0.8) +
  labs(
    x = "Délai entre la greffe et le séjour de suivi",
    y = "Nombre de séjours"
  ) +
  theme_minimal() +
  theme(
...
ggsave(
  "figures/delai_sejours_suivi.png",
  plot = p,
  width = 8,
  height = 6,
  dpi = 300
...
greffes_par_patient <- data %>%
  ...
  distinct(study_id, date_greffe, RSA_NUM_greffe) %>%
...
top_dgn_pal <- sort(table(data_patients$DGN_PAL_suivi), decreasing = TRUE)[1:10]
df_dgn <- data.frame(
...
```{r}
top_dgn_rel <- sort(table(data_patients$DGN_REL_suivi), decreasing = TRUE)[1:10]
...
Compter le nombre de greffes par établissement

```{r}

greffes_par_etab <- data %>%
  group_by(etab_greffe) %>%
  summarise(
    nb_greffes = n(),
    .groups = "drop"
  ) %>%
...
top10_etab <- greffes_par_etab %>%
  slice_head(n = 10)

ggplot(top10_etab, aes(x = reorder(etab_greffe, nb_greffes), y = nb_greffes)) +
  geom_col(fill = "steelblue", color = "black", alpha = 0.8) +
  geom_text(
  ...
  coord_flip() +
  labs(
    title   = "Top 10 des établissements réalisant le plus de greffes rénales",
    x       = "Établissement",
...
# ÉTAPE  : Analyse par département (au lieu d'établissement)
...
greffes_par_dept <- data_patients %>%
  ...
  summarise(
    nb_greffes = n(),
    nb_etablissements = n_distinct(ETA_NUM_greffe)
  ) %>%
  ungroup() %>%
  arrange(desc(nb_greffes)) %>%
  mutate(
...
ggplot(top10_dept, aes(x = reorder(DEP_greffe, nb_greffes), y = nb_greffes)) +
...
data_patients <- data_patients %>%
  ...
  left_join(ref_departements, by = "DEP_greffe")
```

```{r}
# Vérifier la jointure
verif_jointure <- data_patients %>%
  group_by(DEP_greffe, nom_dept, region) %>%
...
greffes_par_dept <- data_patients %>%
  ...
  mutate(
    ...
    dept_label = paste0(nom_dept, " (", DEP_greffe, ")")
  )
...
greffes_par_dept <- greffes_par_dept %>%
  mutate(DEP_greffe = as.character(DEP_greffe))

```

```{r}
carte_greffes <- departements %>%
  left_join(greffes_par_dept, by = c("code" = "DEP_greffe"))
...
greffes_par_region <- data_patients %>%
  filter(!is.na(region)) %>%  # Exclure les départements non identifiés
  group_by(region) %>%
  summarise(nb_greffes = n()) %>%
  ungroup() %>%
  arrange(desc(nb_greffes))
...
```{r}
# 1. Supprimer les colonnes issues d'une jointure précédente
carte_regions <- carte_regions %>% 
  select(-any_of(c("population", "taux_pour_100k")))

# 2. Redéfinir pop_regions
pop_regions <- tibble(
  nom = c("Île-de-France", "Auvergne-Rhône-Alpes", "Nouvelle-Aquitaine",
          "Occitanie", "Grand Est", "Hauts-de-France",
          "Provence-Alpes-Côte d'Azur", "Pays de la Loire", "Normandie",
          "Centre-Val de Loire", "Bretagne", "Bourgogne-Franche-Comté", "Corse"),
  population = c(12271794L, 8094979L, 6120382L, 6156981L, 5579388L,
                 6003085L, 5131040L, 3871098L, 3338004L,
                 2604891L, 3394539L, 2830655L, 344679L)
)

# 3. Jointure propre
carte_regions <- carte_regions %>%
  left_join(pop_regions, by = "nom") %>%
  mutate(
    taux_pour_100k = round(as.numeric(nb_greffes) / as.numeric(population) * 100000, 1)
  )

# 4. Vérification
glimpse(carte_regions)
```

```{r}
ggplot(carte_regions) +
  geom_sf(aes(fill = taux_pour_100k), color = "white", linewidth = 0.3) +
  geom_sf_text(
    aes(label = taux_pour_100k),  # ← manquait
    size = 2.3,
    fontface = "bold",
    color = "white"
  ) +
  ...
  labs(
    title    = "Taux de greffes rénales par région pour 100 000 habitants",
    subtitle = "Ramené à la population régionale (INSEE 2023)",
    caption  = "Source : données hospitalières | Population : INSEE 2023"
  ) +
  theme_minimal() +
  theme(
    plot.title    = element_text(face = "bold", size = 14),
...
obtenir_top_diagnostics <- function(data, n = 10) {
  resultat <- data %>%
    ...
    count(diag_greffe, name = "Occurrences") %>%  
...
duree_age <- duree_par_patient %>%
  left_join(
    data_patients %>% select(study_id, age_greffe),
    by = "study_id"
  )
...
ggplot(duree_age, aes(x = age_greffe, y = duree_totale)) +
...
duree_diagnostic <- duree_par_patient %>%
  left_join(
    data_patients %>% select(study_id, DGN_PAL_suivi),
    by = "study_id"
  )
...
top5_dgn <- data_patients %>%
  count(DGN_PAL_suivi, sort = TRUE) %>%
  slice_head(n = 5) %>%
  pull(DGN_PAL_suivi)

duree_diagnostic_top5 <- duree_diagnostic %>%
  filter(DGN_PAL_suivi %in% top5_dgn)
