...
data <- data %>%
  mutate(
    ETA_NUM_char = as.character(ETA_NUM_greffe),
...
### Enregistrement de la base de données préparée
```{r}
# base rds
saveRDS(data, "data.rds")
```
...
### Création de la variable cancer
Le cancer correspond au diagnostiques principaux commençant par C
```{r}
data <- data %>%
  mutate(
    cancer_sejour = as.integer(str_detect(DGN_PAL_suivi, regex("^c", ignore_case = TRUE)))
  )
