...
convertir_dates <- function(data, variables_dates){
  # Forcer locale anglaise pour reconnaître les mois abrégés
  Sys.setlocale("LC_TIME", "C")
  
  for(var in variables_dates){
    # Extraire la partie date avant les ":"
    clean_var <- paste0(var, "_clean")
    data[[clean_var]] <- sub(":.*", "", data[[var]])
    
    # Conversion en Date
    data[[paste0(var, "_date")]] <- as.Date(data[[clean_var]], format = "%d%b%Y")
    
    # Format jour-mois-année
    data[[paste0(var, "_jma")]] <- ifelse(
      is.na(data[[paste0(var, "_date")]]),
      NA,
      format(data[[paste0(var, "_date")]], "%d-%m-%Y")
    )
