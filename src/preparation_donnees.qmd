...
est_manquant <- function(x) {
  ...
  # conversion sécurisée pour les colonnes caractères
  if (is.character(x)) {
...
ajouter_delais <- function(donnees, study_id = "study_id") {
  ...
  date_fictive <- as.Date("1600-01-01")
  donnees$date_mort[donnees$date_mort == date_fictive] <- NA
  ...
  dates_hospit_comp <- donnees %>%
    ...
    summarise(
      date_premiere_hospit_complication = {
        tmp <- debut_sejour[!is.na(debut_sejour)]
        if (length(tmp) == 0) NA else min(tmp)
      },
      .groups = "drop"
    )
...
# il y a des patients qui ne sont pas morts et qui n'ont pas de suivi après la greffe (perte de trace juste après la greffe => censure immédiate)
 donnees$date_fin_suivi <- case_when(
  ...
  is.na(donnees$date_complication) &
    (!is.na(donnees$fin_sejour) | !is.na(donnees$debut_sejour)) ~
      pmax(donnees$fin_sejour,
           donnees$debut_sejour,
           na.rm = TRUE),

  # Cas perdu de vue immédiatement après greffe
  TRUE ~ donnees$date_greffe
)
...
data <- ajouter_delais(data, study_id = "study_id")
head(data)
...
data[which(data$delai_greffe_complication<0),] %>% select(date_greffe, date_mort, debut_sejour, fin_sejour, DGN_PAL_suivi, study_id)
```
...
```{r}
# Variable évènement
data$event_complication <- ifelse(
  !is.na(data$date_complication),
  1,
  0
)
...
data_patient <- data %>%
  ...
  mutate(
    ...
    DGN_PAL_avant_complication = ifelse(
      !is.na(DGN_PAL_valide) &
      (is.na(date_complication) | debut_sejour <= date_complication),
      DGN_PAL_valide,
      NA
    )
  ...
  ungroup()
data[which(is.na(data$fin_sejour)),]
```

```{r}
# vérifications
summary(data_patient$delai_greffe_complication)
sum(is.na(data_patient$delai_greffe_complication))
sum(data_patient$delai_greffe_complication < 0)

table(data_patient$event_complication)
data_patient[which(is.na(data_patient$delai_greffe_complication)),] # 1 pb, données textuelles plutôt que vraies informations, on supprime la ligne
head(data_patient, 3)
data[which(data_patient$study_id == "patient_1"),] %>% select(date_greffe,debut_sejour, fin_sejour, date_mort, DGN_PAL_suivi)
```
